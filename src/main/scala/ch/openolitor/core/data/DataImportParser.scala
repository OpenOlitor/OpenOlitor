...
class DataImportParser extends Actor with ActorLogging {
  ...
  def importData(file: InputStream): Try[ParseResult] = {
    ...
    for {
      ...
      (tourlieferungen, _) <- Try(doc.withSheet("Tourlieferungen")(parseTourlieferungen(abos, kunden)))
    } yield {
      ParseResult(
        projekt,
        customKundentypen,
  ...
  def parseProjekte = {
    parse[Projekt, ProjektId]("id", Seq("bezeichnung", "strasse", "haus_nummer", "adress_zusatz", "plz", "ort",
      "preise_sichtbar", "preise_editierbar", "email_erforderlich", "waehrung", "geschaeftsjahr_monat", "geschaeftsjahr_tag", "two_factor_auth") ++ modifyColumns) { id =>
      indexes =>
        row =>
  ...
  def parseKunden(personen: List[Person]) = {
    parse[Kunde, KundeId]("id", Seq("bezeichnung", "strasse", "haus_nummer", "adress_zusatz", "plz", "ort", "bemerkungen",
      ...
      "anzahl_abos", "anzahl_pendenzen", "anzahl_personen") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parsePersonen = {
    parse[Person, PersonId]("id", Seq("kunde_id", "anrede", "name", "vorname", "email", "email_alternative",
      "telefon_mobil", "telefon_festnetz", "bemerkungen", "sort", "login_aktiv", "passwort", "letzte_anmeldung", "passwort_wechsel", "rolle") ++ modifyColumns) { id =>
      indexes =>
        row =>
          //match column indexes
  ...
  def parsePendenzen(kunden: List[Kunde]) = {
    parse[Pendenz, PendenzId]("id", Seq("kunde_id", "datum", "bemerkung", "status", "generiert") ++ modifyColumns) { id =>
      indexes =>
        row =>
          //match column indexes
  ...
  def parseDepots = {
    parse[Depot, DepotId]("id", Seq("name", "kurzzeichen", "ap_name", "ap_vorname", "ap_telefon", "ap_email", "v_name", "v_vorname",
      ...
      "plz", "ort", "aktiv", "oeffnungszeiten", "farb_code", "iban", "bank", "beschreibung", "max_abonnenten", "anzahl_abonnenten") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseTours = {
    parse[Tour, TourId]("id", Seq("name", "beschreibung") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseAbotypen = {
    parse[Abotyp, AbotypId]("id", Seq("name", "beschreibung", "lieferrhythmus", "preis", "preiseinheit", "aktiv_von", "aktiv_bis", "laufzeit",
      ...
      "kuendigungsfrist", "vertragslaufzeit", "anzahl_abonnenten", "letzte_lieferung", "waehrung") ++ modifyColumns) { id =>
      indexes => row =>
  ...
  def parseAbwesenheit = {
    parse[Abwesenheit, AbwesenheitId]("id", Seq("abo_id", "lieferung_id", "datum", "bemerkung") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseVertriebsarten = {
    parse[Vertriebsart, VertriebsartId]("id", Seq("vertrieb_id", "depot_id", "tour_id", "anzahl_abos") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseProdukteProduzenten = {
    parse[ProduktProduzent, ProduktProduzentId]("id", Seq("produkt_id", "produzent_id") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseProdukteProduktkategorien = {
    parse[ProduktProduktekategorie, ProduktProduktekategorieId]("id", Seq("produkt_id", "produktekategorie_id") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseProduktekategorien = {
    parse[Produktekategorie, ProduktekategorieId]("id", Seq("beschreibung") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseProduzenten = {
    parse[Produzent, ProduzentId]("id", Seq("name", "vorname", "kurzzeichen", "strasse", "haus_nummer", "adress_zusatz",
      "plz", "ort", "bemerkung", "email", "telefon_mobil", "telefon_festnetz", "iban", "bank", "mwst", "mwst_satz", "mwst_nr", "aktiv") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseLieferplanungen = {
    parse[Lieferplanung, LieferplanungId]("id", Seq("bemerkung", "abotyp_depot_tour", "status") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseLieferpositionen(produkte: List[Produkt], produzenten: List[Produzent]) = {
    parse[Lieferposition, LieferpositionId]("id", Seq("lieferung_id", "produkt_id", "produzent_id", "preis_einheit", "liefereinheit", "menge", "preis", "anzahl") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseVertriebe(vertriebsarten: List[Vertriebsart]) = {
    parse[Vertrieb, VertriebId]("id", Seq("abotyp_id", "beschrieb", "liefertag", "anzahl_abos") ++ modifyColumns) { id =>
      indexes => row =>
  ...
  def parseLieferungen(abotypen: List[Abotyp], vertriebe: List[Vertrieb], abwesenheiten: List[Abwesenheit], lieferplanungen: List[Lieferplanung],
    ...
    parse[Lieferung, LieferungId]("id", Seq("abotyp_id", "vertrieb_id", "lieferplanung_id", "status", "datum", "anzahl_abwesenheiten", "durchschnittspreis",
      "anzahl_lieferungen", "anzahl_koerbe_zu_liefern", "anzahl_saldo_zu_tief", "zielpreis", "preis_total") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseAbos(kundeIdMapping: Map[Long, KundeId], kunden: List[Kunde], vertriebsartIdMapping: Map[Long, VertriebsartId], vertriebsarten: List[Vertriebsart], vertriebe: List[Vertrieb],
    ...
    parse[Abo, AboId]("id", Seq("kunde_id", "vertriebsart_id", "start", "ende",
      ...
      "depot_id", "tour_id") ++ modifyColumns) { id =>
      indexes =>
        row =>
          //match column indexes
  ...
  def parseBestellungen(produzenten: List[Produzent], lieferplanungen: List[Lieferplanung]) = {
    parse[Bestellung, BestellungId]("id", Seq("produzent_id", "lieferplanung_id", "datum", "datum_abrechnung", "preis_total") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseCustomKundentypen = {
    parse[CustomKundentyp, CustomKundentypId]("id", Seq("kundentyp", "beschreibung", "anzahl_verknuepfungen") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
  ...
  def parseTourlieferungen(abos: List[Abo], kunden: List[Kunde]) = {
    parse[Tourlieferung, AboId]("id", Seq("sort") ++ modifyColumns) { id =>
      indexes => row =>
        //match column indexes
