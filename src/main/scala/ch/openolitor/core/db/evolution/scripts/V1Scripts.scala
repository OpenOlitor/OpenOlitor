...
object V1Scripts {
  val DBInitializationScript = new Script with LazyLogging with StammdatenDBMappings {
    def execute(implicit session: DBSession): Try[Boolean] = {
      ...
      sql"create table ${personMapping.table} (id varchar(36) not null, kunde_id varchar(50) not null, anrede varchar(20) null, name varchar(50) not null, vorname varchar(50) not null, email varchar(100) not null, email_alternative varchar(100), telefon_mobil varchar(50), telefon_festnetz varchar(50), bemerkungen varchar(512), sort int not null)".execute.apply()
