...
trait StammdatenJsonProtocol extends BaseJsonProtocol with ReportJsonProtocol with LazyLogging with AutoProductFormats[JSONSerializable] {
  ...
  implicit val hauptAbotypIdFormat = baseIdFormat(HauptAbotypId)
  implicit val zusatzAbotypIdFormat = baseIdFormat(ZusatzAbotypId)
  ...
  implicit val hauptAboIdFormat = baseIdFormat(HauptAboId)
  implicit val ZusatzAboIdFormat = baseIdFormat(ZusatzAboId)
